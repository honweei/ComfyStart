...
if not Path(WORKSPACE).exists():
    run_command(f"git clone https://github.com/comfyanonymous/ComfyUI", "Step 2/8: Cloning ComfyUI repository")
...
if OPTIONS['INSTALL_FLUX']:
    run_command(f"cd {WORKSPACE}/custom_nodes && git clone https://github.com/city96/ComfyUI-GGUF", "Step 3/8: Installing FLUX-GGUF node")
...
if OPTIONS['INSTALL_COMFYUI_MANAGER']:
    if not Path(f"{WORKSPACE}/custom_nodes/ComfyUI-Manager").exists():
        run_command(f"cd {WORKSPACE}/custom_nodes && git clone https://github.com/ltdrdata/ComfyUI-Manager", "Step 4/8: Installing ComfyUI-Manager")
...
if OPTIONS['INSTALL_FLUX_API']:
    if not Path(f"{WORKSPACE}/custom_nodes/comfyscope").exists():
        run_command(f"cd {WORKSPACE}/custom_nodes && git clone https://github.com/modelscope/comfyscope.git", "Step 5/8: Installing FLUX-API node")
